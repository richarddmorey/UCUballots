---
title: "UCU 2018/2019 pension/pay ballot report"
...
readxl::excel_sheets(data_file)[1:6] %>%
...
all_data %>%
  ...
  arrange(desc(Mailedout_2018)) -> year_summaries_pension

all_data %>%
  filter(Year == 2019, Action == "strike") %>%
  select(-Year, -Action) %>%
  pivot_wider(id = University, 
              names_from = Type, 
              values_from = c(p_yes, p_turnout, Mailedout)) %>% 
  mutate( University = htmltools::htmlEscape(University, attribute = TRUE)) %>%
  arrange(desc(Mailedout_pay)) -> summaries_2019

all_data %>% 
  filter(Action == "strike", Type == "pension", Year == "2019") %>%
  select(University) %>% 
  unlist() %>% 
  unname() -> pension_unis
...
In the report below, I outline some observations about the UCU ballots on pensions and pay in 2018 and 2019. The figures are generally interactive; mouse over the points to see what University is represented. 

I have put the data in a convenient `xlsx` format (unlike the inconvenient UCU data PDFs); this can be found on GitHub, along with the R source code for this report (https://github.com/richarddmorey/UCUballots). I have also created an interactive applet for exploring the ballot results at https://rdmorey.shinyapps.io/ucuballots/.
...
All analyses are of the strike ballots; the `xlsx` file also contains data on the action-short-of-strike ballots, but the results are broadly similar and so are omitted here. Both the data table at the end of this document and the interactive app (https://rdmorey.shinyapps.io/ucuballots/) allow exploration of the action-short-of-strike ballot results.

## Comparisons between pension ballots, 2018/2019

### Ballots mailed

Membership appears to be up from 2018 by about 15% on average. A notable exception to this among the larger universities is University College London, which saw over 50% more ballots sent out in 2019 compared to 2018.

```{r}
year_summaries_pension %>%
  ggplot(aes(x = Mailedout_2018, y = Mailedout_2019, size = Mailedout_2018, 
  ...
  scale_size_area(name = "Mailed out, 2018") +
  coord_fixed(ratio = 1, expand = FALSE, clip = "off") +
  geom_smooth(method = loess, se = FALSE, mapping = aes(weight = Mailedout_2018, group = 1)) +
  geom_abline(slope = 1, intercept = 0, alpha = .3, linetype = "dashed") +
  guides(size = guide_legend(override.aes = list(linetype = 0))) +
  theme_minimal() +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  ylab("Ballots mailed, 2019") +
  xlab("Ballots mailed, 2018") +
  ggtitle("Pension (USS) ballots mailed, 2018 vs 2019") -> p
...
Average turnout across universities for the pension ballot was down by about 6% on average from 2018 to 2019.

```{r}
year_summaries_pension %>%
  mutate(p_turnout_2018 = p_turnout_2018 * 100,
         p_turnout_2019 = p_turnout_2019 * 100) %>%
  ggplot(aes(x = p_turnout_2018, y = p_turnout_2019, size = Mailedout_2018,
             tooltip = University, data_id = University)) +
  ...
  geom_smooth(method = loess, se = FALSE,
              mapping = aes(weight = Mailedout_2018, group = 1)) +
  geom_abline(slope = 1, intercept = 0, alpha = .3, linetype = "dashed") +
  guides(size = guide_legend(override.aes = list(linetype = 0))) +
  theme_minimal() +
  theme(panel.grid.major = element_blank(), 
  ...
  ylab("Turnout, 2019 (%)") +
  xlab("Turnout, 2018 (%)") +
  ggtitle("Pension (USS) ballot turnout, 2018 vs 2019") -> p
...
There were some people I talked to in 2018 who worried that the push for members would affect future votes, decreasing turnout by recruiting people who were less enthusiastic than those members who voted for the 2018 strikes. Although membership *did* increase from 2018 to 2019, and turnout *did* decrease from 2018 to 2019, this pattern does not hold true on the level of universities.

```{r}
year_summaries_pension %>%
  mutate(inc_mailedout = 100 * Mailedout_2019 / Mailedout_2018,
         inc_turnout = 100 * p_turnout_2019 / p_turnout_2018) %>%
  ggplot(aes(x = inc_mailedout, y = inc_turnout, size = Mailedout_2018, 
             tooltip = University, data_id = University)) +
  geom_point_interactive(alpha = .3, stroke = 0, color = "blue") +
  scale_size_area(name = "Mailed out, 2018") +
  ...
  ggtitle("Turnout in 2019 pension (USS) ballot, by membership increase") -> p
...
year_summaries_pension %>%
  mutate(inc_mailedout = 100 * Mailedout_2019 / Mailedout_2018,
         inc_turnout = 100 * p_turnout_2019 / p_turnout_2018) %>% select( matches("inc_")) %>%
...
We can also compare the proportion of "yes" votes across the two years. In both 2018 and 2019, votes were *overwhelmingly* in favor of striking. In the 2018 ballot, the average "yes" vote across universities was `r yes_votes$p_yes[1]`%. In 2019, it was lower but still very high at `r yes_votes$p_yes[2]`%. In neither year did *any* university fail to meet a 50% "yes" threshold for industrial action. 

```{r}

year_summaries_pension %>%
  mutate(p_yes_2018 = p_yes_2018 * 100,
         p_yes_2019 = p_yes_2019 * 100) %>%
  ggplot(aes(x = p_yes_2018, y = p_yes_2019, size = Mailedout_2018, 
             tooltip = University, data_id = University)) +
  geom_abline(slope = c(.8, .9, 1.1, 1.2), intercept = 0, alpha = .2, linetype = "dotted") +
  ...
  ylab("Yes vote, 2019 (%)") +
  xlab("Yes vote, 2018 (%)") +
  ggtitle("Pension (USS) ballot yes votes, 2018 vs 2019") -> p
...
The table below shows the number of universities that had/have the opportunity to take industrial action in each year. Although the same number of universities are involved, and there has not been much change in turnout, and members overwhemlingly favor action, the result is a 25% decline in the number of universities that can take part in the industrial action from 2018 to 2019.
...
Another way of looking at it is by looking at the proportion of members at universities that cannot take part. In 2018, `r member_failed$percent[2]`% of members were not allowed to take part in the industrial action; in 2019, this number is `r member_failed$percent[4]`%. Assuming membership is roughly proportional to the size of the instututions, this is a dramatic change in the number of employees that can be take part in strike action.

*Why* the dramatic change? It is all down to how thresholds work (and in this case, are meant to work). If there are a sizeable number of universities just above the turnout threshold&mdash;and there will be when turnout is in the mid-50%s&mdash;then a small change in turnout can translate into a effect on the industrial action.

But have things changed that much? Not really. If we look at the overall ballot numbers (figure below), it seems the major change is in membership. The number of "yes" votes has barely changed, and to the extent that the populations of these instututions has remained the same, this means that the proportion of employees voting to strike has remained constant. 
...
all_data %>%
  ...
  ylim(c(0,50000)) +
  ggtitle("2018/2019 pension ballot totals") +
  theme_minimal() +  
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) 
...
## Pay vs pensions, 2019

Because there were two votes in 2019&mdash;a vote on striking over pensions and a vote on striking over pay and casualization&mdash;we can look at the relationship between these two votes.
...
summaries_2019 %>%
  ...
  ggtitle("Pay vs. pension (USS) ballot turnout, 2019") -> p
...
summaries_2019 %>%
  ...
  ggtitle("Pay vs. pension (USS) ballot yes votes, 2019") -> p
...
Of course, these are the results only for those universities that were ballotted for both. Of these universities, the results for the pension ballot and the pay ballot are very similar. Yet only about 37% of universities will be allowed to strike over pay, compared to 68% for pensions. What happened?

The table below shows the ballot results broken down by ballot type, as well as whether the university was *also* ballotted for pension. Those universities ballotted for both tended to meet the threshold at the same rate (about 68%). But the universities *not* ballotted for pensions tended not to meet the threshold (only 13% did).

```{r}
all_data %>%
  mutate(pension_uni = University %in% !!pension_unis,
         pension_uni = factor(pension_uni, 
                              labels = c("Not ballotted for pensions", "Ballotted for pensions"))) %>%
  filter(Year == "2019", Action == "strike") %>%
...
all_data %>%
  ...
  ggtitle("Pay turnout by whether ballotted for pensions") -> p
...
all_data %>%
  ...
  ylab("Yes (%)") +
  xlab("Also balloted for pensions?") +
  ggtitle("Pay ballot vote by whether ballotted for pensions") -> p
